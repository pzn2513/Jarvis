...
特殊区域(){
    ...
    mwaitfunc([
        ...
        特殊区域_上路],
        [()=>mgetpix([[1429,903,0x1A2028,16], [1321,886,0xEB2429,32]]),特殊区域_下路],
    ],"路线分析")
}
零号业绩(){
    零号业绩_start()
    ...
    mwaitfunc([
        [()=>mgetpix([[1440,67,0x353C44,16], [1319,226,0xF63C3D,32]]),
        零号业绩_上路],
        [()=>mgetpix([[1429,903,0x1A2028,16], [1321,886,0xEB2429,32]]),零号业绩_下路],
    ],"路线分析")
/**
 *  右上                红点
 * 1440,67,0x353C44 1319,226,0xF63C3D
 * 1400,67,0x0A1427 1319,226,0x475F6B
 * 
 * 1234,738,0x21262B 1429,903,0x1A2028 1321,886,0xEB2429
 * 1234,738,0x1D2228 1429,903,0x171F28 1321,886,0xE5262B
 */
}
零号业绩_start(){
...
零号业绩_get_reward(){
    ...
    click(1490,734)
    sleep 100
    waitpix 1577,130,0xB83736,32,3,()=>click(961,807) ; s门,点掉对白
    waitpix 1291,363,0x343334,6,1,()=>click(1577,130),1000 ;s门对话框，前往s门
    waitpix 1291,363,0x343334,6,-8,()=>click(1291,363) ;s门对话框，点掉对白
    res:=mwaitpix([[1084,311,0x6C54A1,16],[1222,307,0x775FB4,16]],0,1,,100,2500)
    if(res=="timeout"){
